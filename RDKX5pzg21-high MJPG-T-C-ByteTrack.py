...
集成 YOLOv8n(BPU) + ByteTrack 多目标跟踪（持续检测模式，可点击选择目标ID）
新增：跟踪模式下按 C 键保存跟踪视频（含边界框与ID）
"""

import sys
# 将 rdk_model_zoo 根目录加入模块搜索路径（确保在导入 ultralytics_yolo_det 之前）
...
# 配置 UltralyticsYOLODetect 所需的参数（降低检测阈值）
cfg = SimpleNamespace(
    model_path=BPU_MODEL_PATH,
    classes_num=80,
    score_thres=0.3,          # 降低到0.3，让更多低分框进入跟踪器
    nms_thres=0.45,
    resize_type=1,
    reg=16,
    strides=[8, 16, 32],
...
def main():
    ...
    print("="*70)
    print("偏振镜控制系统 + YOLOv8n(BPU) + ByteTrack多目标跟踪")
    print("RDK X5 移植版 - 使用BPU加速 + ByteTrack跟踪")
    print("新增功能：跟踪模式下按 C 键开始/停止保存跟踪视频（含边界框与ID）")
    print("模式说明：")
    print("  - SCAN: 按 A 启动偏振扫描，扫描后自动回到 SCAN")
    print("  - SCAN 状态下按 T：进入目标检测模式（持续检测，每帧检测）")
    print("  - 检测模式下点击任意框选择目标ID，切换到跟踪模式（高亮该目标）")
    ...
    # ==================== ByteTrack 跟踪器初始化（放宽参数） ====================
    byte_tracker = BYTETracker(
        track_thresh=0.3,          # 降低跟踪阈值，允许低分框
        track_buffer=60,           # 增加缓存，允许更长时间丢失
        match_thresh=0.5,         # 降低匹配阈值，提高鲁棒性
        frame_rate=30
    )
    print("ByteTrack 跟踪器初始化成功")

    # ==================== 用于保存 track_id 对应的类别 ====================
    track_class_map = {}  # track_id -> class_id

    selected_track_id = -1   # 用户选择的跟踪目标ID，-1表示未选择
    ...
    select_data = {'boxes_orig': [], 'track_ids': [], 'scale_x': 1.0, 'scale_y': 1.0}

    # ==================== 鼠标回调：点击选择 track_id ====================
    def mouse_callback(event, x, y, flags, param):
        nonlocal selected_track_id, current_mode
        if current_mode != MODE_TRACK_SELECT:
            return
        if event == cv2.EVENT_LBUTTONDOWN:
            boxes = param.get('boxes_orig', [])
            track_ids = param.get('track_ids', [])
            if not boxes:
                return
            ...
            for i, box in enumerate(boxes):
                ...
                if x1 <= orig_x <= x2 and y1 <= orig_y <= y2:
                    if i < len(track_ids):
                        ...
                        print(f"选中跟踪目标 ID: {selected_track_id}")
    ...
    def detection_worker():
        nonlocal latest_detection, detection_running, track_class_map
        while detection_running:
            time.sleep(0.01)  # 稍微降低CPU占用
            with detection_lock:
            ...
            try:
                ...
                if len(boxes) == 0:
                    ...
                    # 2. 转换为 ByteTrack 输入格式 [x1, y1, x2, y2, score]
                    detections = []
                ...
                tracked_objects = byte_tracker.update(
                    ...
                    img_info=[frm.shape[0], frm.shape[1]],
                    img_size=[frm.shape[0], frm.shape[1]]
                )

                # 4. 解析跟踪结果（STrack 对象列表）
                orig_boxes = []
                track_ids = []
                labels = []

                # 为每个跟踪框分配类别（通过IoU匹配检测框）
                if len(boxes) > 0:
                    for track in tracked_objects:
                        ...
                        # 计算与所有检测框的IoU，选择最大且>0.3的类别
                        best_iou = 0.0
                        best_cls = -1
                        track_box = np.array([x1, y1, x2, y2])
                        for i, det_box in enumerate(boxes):
                            ix1 = max(x1, det_box[0])
                            iy1 = max(y1, det_box[1])
                            ix2 = min(x2, det_box[2])
                            iy2 = min(y2, det_box[3])
                            inter = max(0, ix2 - ix1) * max(0, iy2 - iy1)
                            union = (x2 - x1)*(y2 - y1) + (det_box[2]-det_box[0])*(det_box[3]-det_box[1]) - inter
                            iou = inter / union if union > 0 else 0
                            if iou > best_iou:
                                best_iou = iou
                                best_cls = class_ids[i]
                        if best_iou > 0.3:  # 降低IoU阈值，更宽容
                            track_class_map[track_id] = best_cls
                        cls_id = track_class_map.get(track_id, -1)

                        x1 = max(0, min(frm.shape[1], int(x1)))
                        y1 = max(0, min(frm.shape[0], int(y1)))
                        x2 = max(0, min(frm.shape[1], int(x2)))
                        y2 = max(0, min(frm.shape[0], int(y2)))
                        
                        orig_boxes.append((x1, y1, x2, y2))
                        track_ids.append(track_id)
                        if cls_id != -1 and cls_id < len(COCO_NAMES):
                            label = f"{COCO_NAMES[cls_id]} ID:{track_id} {score:.2f}"
                        else:
                ...
                with detection_lock:
                    ...
                    latest_detection['track_ids'] = track_ids
    ...
    while not exit_flag:
        ...
        if current_mode == MODE_SCAN:
            ...
            with detection_lock:
                ...
                track_ids = latest_detection.get('track_ids', [])
            select_data['boxes_orig'] = orig_boxes
            select_data['track_ids'] = track_ids
            ...
            for i, box in enumerate(boxes_scaled):
                ...
                if selected_track_id != -1 and track_id != selected_track_id:
                    # 非选中目标用灰色半透明显示
                    cv2.rectangle(disp, (x1, y1), (x2, y2), (100, 100, 100), 1)
                    continue
                # 高亮选中的目标（或全部目标）
                color = (0, 255, 255) if track_id == selected_track_id else (0, 255, 0)
            ...
            # 如果选择了目标但当前帧没有该ID，清除选择
            if selected_track_id != -1 and selected_track_id not in track_ids:
                print(f"目标 ID {selected_track_id} 丢失，自动退出跟踪模式")
                selected_track_id = -1
                current_mode = MODE_TRACK_SELECT
                continue
            ...
            if track_video_recording and track_video_writer is not None:
                ...
                for i, box in enumerate(orig_boxes):
                    ...
                    if selected_track_id != -1 and track_id != selected_track_id:
                        continue
                    color = (0, 255, 255) if track_id == selected_track_id else (0, 255, 0)
                    cv2.rectangle(orig_frame_with_track, (x1, y1), (x2, y2), color, 2)
                    if i < len(labels):
                        cv2.putText(orig_frame_with_track, labels[i], (x1, y1-5),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            ...
            if selected_track_id != -1:
                osd_lines.append(f"Tracking ID: {selected_track_id}")
            else:
                osd_lines.append("Tracking all targets")
            cv2.imshow('Live Video', put_osd_for_display(disp, osd_lines, scale=0.6))
            cv2.imshow('Best Frame', get_best_frame_for_display())
