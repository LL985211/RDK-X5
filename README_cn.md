...
本项目面向水面、湿滑路面、玻璃、金属、车窗等强反光场景，构建了一套嵌入式自适应偏振成像与视觉追踪系统。系统以地瓜派 RDK X5 作为视觉计算与任务调度主控，STM32 作为偏振机构实时控制器，结合相机采集、偏振角寻优、图像质量评价、YOLOv8n BPU 推理、DaSiamRPN 单目标跟踪、视频录制和可选 RTSP 推流，实现“光学抑光、图像评价、目标检测、目标跟踪、结果归档”的闭环流程。

当前目录包含：

| 文件 | 说明 |
| --- | --- |
| `RDKX5pzg21-high MJPG-T-C-RPN.py` | RDK X5 端主程序，基于 YOLOv8n BPU 检测和 DaSiamRPN 跟踪实现采集、评分、检测、跟踪、录制和推流。 |
| `2026嵌入式大赛应用赛道作品.docx` | 参赛作品文档，包含系统设计、硬件组成、软件流程和实验结果。 |
| `13ca1b07ab06b731c25ccccd37dcd7a2.mp4` | 项目演示视频，MP4 封装，视频编码为 AVC/H.264。 |
...
- 使用 YOLO 检测框辅助目标选择，再由 DaSiamRPN 执行单目标跟踪。
- 跟踪模式下可录制带目标框的视频。
...
- **RDK X5**：负责相机采集、图像质量评价、BPU 推理、DaSiamRPN 跟踪、界面显示、视频保存、数据导出和 RTSP 推流。
...
1. `SCAN`：偏振扫描与最佳帧选择。
2. `DETECT`：持续运行 YOLO 检测并显示可选择的目标框。
3. `TRACKING`：使用 DaSiamRPN 跟踪选中的目标，并可录制跟踪视频。
...
- OpenCV 跟踪接口，包括 DaSiamRPN 和 CSRT 备选跟踪器支持
...
| `SCAN` | 按 `A` 启动一轮偏振扫描。 |
| `SCAN` | 按 `T` 进入持续目标检测模式。 |
| `DETECT` | 鼠标点击任意检测框，初始化跟踪器并进入跟踪模式。 |
...
文档还记录了强反光场景下的跟踪效果对比：普通 RGB 输入的跟踪 IoU 为 48.5%，经过自适应偏振增强后提升到 67.5%；当前文档中的跟踪模块已按 SiamRPN 方案描述。
